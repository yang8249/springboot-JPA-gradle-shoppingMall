...
public class CartService {
	...
	public Cart cartInfo(int id, int i) {
		return cartRepository.customFindByCart(id, i).orElse(null);
	}

	//현재 로그인된 계정에 대한 장바구니목록 불러오기
	@Transactional(readOnly = true)
	public List<Cart> userCartList(int id) {
		return cartRepository.findUserIdCartList(id).orElse(null);
	}
